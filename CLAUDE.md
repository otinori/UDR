...
UDR 運用 skill 群は `.claude/skills/` に配置済み（**このディレクトリ自体が source of truth**。`plugins/udr/skills/` 等はここから CI が生成するミラー）。AGENTS.md §2.3 の検知フローに従って候補を検知したら、以下の slash コマンドで記録・管理する:

| コマンド | 用途 |
|---|---|
| `/udr-init` | `.udr/` 構造を初期化（未初期化プロジェクトで最初に実行） |
...
| `/udr-review` | 品質棚卸し（orphan / ステータス不整合 / AI 承認待ち / 棄却理由欠落 / DAG・YAML 健全性 / sync 陳腐化 / code_refs 未記入 / 放置 draft の 9 観点） |
| `/udr-dashboard` | 判断状況を `udr_dashboard.html` に出力（集計 / 一覧 / 棄却理由 / 依存関係、自己確認用） |

**[UDR 候補] 提示時の action ボタン**: `AGENTS.md §2.3` の処理フローで応答末尾に付記する `Y/N/D` はそれぞれ `/udr-record` / スキップ / `/udr-search` に対応。

`/udr-record` 等は `.udr/records/` 配下を読み書きする（`UDR-UDR-*.yaml`、`.claude/skills/_udr-shared/CONVENTIONS.md` 準拠）。

### Gemini CLI / Codex CLI / 他ツールとの共存

Gemini CLI は `.claude/skills/_udr-shared/templates/gemini-prompts/` テンプレートを参照して操作する。
Codex CLI は `.claude/skills/_udr-shared/templates/codex-prompts/` テンプレートを参照する。
Cursor 等は `.claude/skills/` を読まないため、AGENTS.md §4 の手動 YAML テンプレートで直接 `.udr/records/<id>.yaml` を作る。
相互運用のため、同じ `.udr/records/` を全ツールで共有する。衝突回避のため ID 採番時は既存ファイル一覧を必ず確認。

> **skill を編集した場合**: `.claude/skills/` を直接編集する（ここが正）。PR を出すと CI（`sync-mirrors.yml`）が `plugins/udr/skills/` 等のミラーを自動生成して bot commit する。手元で `build.sh` を実行する必要は基本的にない。

### ドッグフーディング方針

本プロジェクトは UDR の本家実装であり、**UDR 自身の開発判断も UDR として記録する**。要求定義・アーキテクチャ選定・skill 設計方針等の判断は、検知次第 `/udr-record` で起票すること。
